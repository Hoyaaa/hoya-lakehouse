...
class MenuAdapter(private val onClick: (Menu) -> Unit) :
        ...
        fun bind(menu: Menu) {
            ...
            if (!menu.imagePath.isNullOrEmpty()) {
                ...
                Glide.with(menuImageView.context)
                    .load(storageRef)
                    .placeholder(R.drawable.kkui_store_image)
                    .error(R.drawable.kkui_store_image)
                    .into(menuImageView)
            } else {
                Log.w("MenuAdapter", "imagePath 없음, 기본 이미지 사용")
                menuImageView.setImageResource(R.drawable.kkui_store_image)
            }
        }
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): MenuViewHolder {
        val view = LayoutInflater.from(parent.context)
            .inflate(R.layout.item_menu_kkui, parent, false)
