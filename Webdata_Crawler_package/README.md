...
나라장터(G2B) 사이트를 통해 입찰공고를 모니터링하고 데이터를 정리하는 실무 과정에는 소모적인 단순 반복 작업이 많았습니다.
첫째, 엑셀에 취합된 타겟 공고번호를 웹사이트 검색창에 일일이 입력하고 상세 페이지에 진입하여 공고기관, 사업금액, 시작/종료일시 등 필수 정보를 찾아 복사해오는 수작업에 많은 시간이 소요되었습니다.
둘째, 사이트에서 일괄 다운로드한 입찰공고목록 엑셀 파일에는 실제 업무와 무관한 공고(공사, 납품, 임대, 폐기물, 철거 등)가 대다수 섞여 있어, 매번 수동으로 수십 개의 키워드를 찾아 행을 지워내는 비효율적인 전처리 과정이 강제되는 상황이었습니다.

2. 프로젝트 기획 동기

매일 발생하는 단순 웹 검색, 데이터 복사, 엑셀 키워드 필터링 작업은 실무자의 피로도를 높이고 핵심 업무에 투입되어야 할 시간을 빼앗는 주요 원인이었습니다.
정형화된 패턴으로 반복되는 작업인 만큼, 사람이 직접 눈으로 보고 클릭하는 과정을 소프트웨어로 대체한다면 단순 반복 시간과 휴먼 에러를 동시에 줄일 수 있을 것이라 판단했습니다.
이에 파이썬을 활용해 브라우저 조작(Selenium)과 엑셀 데이터 전처리(Pandas)를 자동화하여, 불필요한 공고는 사전에 걸러내고 필요한 공고의 상세 정보만 자동으로 추출해 문서화하는 스크립트 패키지를 구축하고자 이 프로젝트를 기획함.
