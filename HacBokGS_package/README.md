...
---
1. 문제 정의 (어떤 문제를 해결하고자 했는가?)

11시 ~ 1시 점심시간이 되면 전 학생들이 학생복지회관으로 모이면서 여러 문제점들이 발생했습니다.
가장 큰 문제는 키오스크의 긴 줄과 부족한 좌석이었습니다. 주문을 했지만 앉을 자리가 없어서 음식을 든 채 돌아다니거나, 반대로 자리는 잡았지만 사람이 너무 많아 주문을 하러 가기 힘든 상황이 매일 반복되었습니다.
또한, 혼잡한 공간에서 음식을 갖고 이동하기 불편한 점, 주문 번호가 특정 모니터에서만 표시되어 자리에서는 조리 완료 여부를 확인하지 못하는 상황에 대한 불만이 다수 접수되었습니다.
여기에 더해 신입생들의 경우 학생복지회관의 위치 경로나 식당 외 기타 시설들의 정보를 직관적으로 확인 할 수 없는 문제점도 함께 드러났습니다.

2. 문제 검증

이러한 문제점들이 실제 사용자들의 공통된 불편함인지 확인하기 위해, 교내 학생복지회관 실제 사용자 194명을 대상으로 온라인 설문조사를 진행하였고, 앞서 도출한 문제점들이 실제 가장 큰 페인포인트임을 데이터로 파악 가능하였습니다.

3. 프로젝트 기획 동기 (왜 만들었는가?)

설문조사를 통해 발견한 오프라인 환경의 불편함과 정보 부족 문제들을 보며, 현재 상황에 맞춰 좀 더 모바일 및 소프트웨어적으로 접근해보고자 했습니다.
단순한 시설 확충이 아닌, 학생들이 개인의 스마트폰을 통해 언제 어디서든 메뉴를 주문하고, 자리에서 조리 상태를 확인하며, 혼잡도를 미리 파악하는 등 지금보다 훨씬 편리하게 학생복지회관을 사용 할 수 있는 실질적인 방법을 찾고자 이 프로젝트를 기획함.
---
## 📁 폴더 구조
