...
class KimchiJjimMenuJeongsikFragment : DialogFragment() {
    private val TAG = "TaesanMenuJeongsikFragment"
    ...
    override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
        ...
        val menuPrice = menu.size?.jeongsik?.price?.toIntOrNull() ?: 0
        var amount = 1
    ...
    private fun updateUI(menu: Menu) {
        ...
        binding.menuPrice.text = if (!jeongsikPrice.isNullOrEmpty()) {
            "정식: ${jeongsikPrice}원"
        } else {
            "가격 정보 없음"
        }
