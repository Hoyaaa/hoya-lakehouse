...
async function loadStoreData() {
  ...
  const storeId = "taesankimchijjim";
  ...
  try {
    ...
    if (!storeSnap.exists()) {
      console.error("❌ taesankimchijjim 문서가 존재하지 않습니다.");
...
function showSaveButton() {
  ...
  addBtn.addEventListener("click", () => {
    window.open("taesan_add.html", "메뉴 추가", "width=1200,height=800");
  });

  delBtn.addEventListener("click", () => {
    window.open("taesan_del.html", "메뉴 삭제", "width=800,height=600");
  });
}
