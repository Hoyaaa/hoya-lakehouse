...
categorySelect.addEventListener("change", async () => {
  ...
  try {
    const menuColRef = collection(db, `store/syongsyongdonkacheu/${category}`);
...
// 🔹 삭제 버튼 클릭 시 Firestore + Storage 동시 삭제
deleteButton.addEventListener("click", async () => {
  ...
  try {
    const docRef = doc(db, `store/syongsyongdonkacheu/${category}`, menuId);
    ...
    console.log(`📄 Firestore 메뉴 '${menuId}' 삭제`);

    if (imagePath) {
      const imageRef = ref(getStorage(), imagePath);
      await deleteObject(imageRef);
      console.log(`🖼 Storage 이미지 '${imagePath}' 삭제`);
    }
