...
document
  ...
  .addEventListener("change", async (e) => {
    ...
    if (!file || !id) {
      alert("먼저 메뉴 ID를 입력한 후 이미지를 선택하세요.");
      return;
    }

    const storagePath = `store/syongsyongdonkacheu/${category}/${id}.png`;
...
document.getElementById("add-button").addEventListener("click", async () => {
  ...
  try {
    await setDoc(doc(db, `store/syongsyongdonkacheu/${category}`, id), data);
